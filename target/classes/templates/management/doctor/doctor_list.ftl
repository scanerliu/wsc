...
<body class="mainbody">
<form method="post" action="/management/doctor/list" id="form1">
...
<div id="floatHead" class="toolbar-wrap" style="height: 52px;">
  <div class="toolbar">
    <div class="box-wrap">
      ...
      <div class="l-list">
        <ul class="icon-list">
          <#-- <li><a class="add" href="/management/doctor/edit"><i></i><span>新增</span></a></li>
        ...
        <div class="menu-list">
          ...
          <select name="statusId" onchange="javascript:setTimeout('__doPostBack(\'isOnline\',\'\')', 0)" id="ddlCategoryId" style="display: none;">
            	<option value="">全部</option>
                  <option value="1" <#if param.statusId?? && param.statusId==1>selected="selected"</#if>>在线</option>
                  <option value="0" <#if param.statusId?? && param.statusId==0>selected="selected"</#if>>离线</option>
            </select>
          </div>
        </div>
      </div>
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="ltable">
    ...
    <#if doctor_page??>
      ...
      <tr>
        ...
        -->
        <td align="center">
          <a href="/management/doctor/del?id=${item.id?c}">删除</a>
          <a href="/management/doctor/edit?id=${item.id?c}">编辑</a>
        </td>
      </tr>
      </#list>
    </#if>
