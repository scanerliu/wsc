...
<section class="center_top wrap100" style="height: 2.9rem;">
    ...
    <section class="login_header">
        ...
        <div class="htitle header colfff">积分明细</div>
...
<section class="purse_list">
   ...
   <#list point_list as point>
   	<#if todayS != point.pointTime?string('dd')>
   	<#assign todayS=point.pointTime?string('dd')>
   	<#if point_index == 0>
    <dl>
    <#else>
    </dl>
    <dl>
        ...
        <dt>${point.pointTime?string('yyyy-MM-dd')}</dt>
        <dd>
            <span>${point.detail!''}</span>
            <label class="purse_list_add">${point.point}分</label>
        </dd>
    <#else>
    	<dd>
            <span>${point.detail!''}</span>
