            ...
            uploader.on('uploadSuccess', function (file, data) {
                ...
                if (data.status == '1') {
                    ...
                    if (!p.multiple) {
                        ...
                        parentObj.siblings(".photo-list").html(newLi);
                    } else {
...
function addImage(targetObj, originalSrc, thumbSrc) {
    //插入到相册UL里面
    var newLi = $('<li>'
    //+ '<input type="hidden" name="picUris" value="0|' + originalSrc + '|' + thumbSrc + '" />'
    + '<input type="hidden" name="showPictures" value="' + originalSrc + '" />'
