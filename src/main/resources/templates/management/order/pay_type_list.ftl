...
<body class="mainbody">
<form method="post" action="/management/pay/list" id="form1">
...
<div id="floatHead" class="toolbar-wrap" style="height: 52px;">
  <div class="toolbar">
    <div class="box-wrap">
      ...
      <div class="l-list">
        <ul class="icon-list">
          <li><a class="add" href="/management/pay/edit"><i></i><span>新增</span></a></li>
...
<div class="table-container">
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="ltable">
    <tbody><tr class="odd_bg">
      ...
      <th width="16%" align="center">在线支付</th>
    ...
    <#if pay_type_page??>
      ...
      <tr>
        ...
        <td><a href="/management/pay/edit?id=${item.id?c}">${item.title!''}</a></td>
        <td align="center">${item.code!}</td>
        <td align="center"><#if item.isEnable??><#if item.isEnable>开启<#else>关闭</#if></#if></td>
        <td align="center"><#if item.isOnlinePay??><#if item.isOnlinePay>是<#else>否</#if></#if></td>
        <td><input name="sortId" type="text" value="${item.sortId!''}" class="sort" onkeydown="return checkNumber(event);"></td>
        <td align="center">
          <a href="/management/pay/edit?action=Copy&id=${item.id?c}">复制</a>
          <a href="/management/pay/edit?id=${item.id?c}">修改</a>
        </td>
      </tr>
      </#list>
    </#if>
