...
<body class="mainbody">
<form method="post" action="/management/express/save" id="form1">
<div class="aspNetHidden">
<input type="hidden" name="__VIEWSTATE" id="__VIEWSTATE" value="">
<input type="hidden" name="id" value="<#if express_type??><#if !action?? || action!="Copy">${express_type.id?c}</#if></#if>">
</div>
<!--导航栏-->
<div class="location">
  <a href="/management/express_type/list" class="back"><i></i><span>返回列表页</span></a>
  <a href="/management/center.aspx" class="home"><i></i><span>首页</span></a>
  <i class="arrow"></i>
  <a href="/management/express_type/list"><span>内容管理</span></a>
  <i class="arrow"></i>
  <span>编辑内容</span>
</div>
...
<div class="tab-content" style="display: block;">
  <dl>
    <dt>名称</dt>
    <dd>
      <input name="title" type="text" value="<#if express_type??>${express_type.title!''}</#if>" class="input normal" datatype="*2-100" sucmsg=" ">
    ...
    <dd>
      <div class="rule-multi-radio">
        <span style="display: none;">
          <input type="radio" name="isEnable" value="1" <#if !express_type?? || !express_type.isEnable?? || express_type.isEnable>checked="checked"</#if>><label>开启</label>
          <input type="radio" name="isEnable" value="0" <#if express_type?? && !express_type.isEnable>checked="checked"</#if>><label>关闭</label>
        </span>
      </div>
    </dd>
  </dl>
  <dl>
    <dt>编码</dt>
    <dd>
      <input name="code" type="text" value="<#if express_type??>${express_type.code!}</#if>" class="input normal" datatype="*0-100" sucmsg=" ">
  ...
  <dl>
    <dt>物流网址</dt>
    <dd>
      <input name="uri" type="text" maxlength="255" value="<#if express_type??>${express_type.uri!''}</#if>" class="input normal">
  ...
  <dl>
    <dt><span>费用</span></dt>
    <dd>
      <input name="fee" type="text" value="<#if express_type??>${express_type.fee!}<#else>0.00</#if>" class="input small" datatype="/^(([1-9]{1}\d*)|([0]{1}))(\.(\d){1,2})?$/" sucmsg=" "> 元
      <span class="Validform_checktip">配送费用</span>
    </dd>
  </dl>
  <dl>
    <dt>排序数字</dt>
    <dd>
      <input name="sortId" type="text" value="<#if express_type??>${express_type.sortId!'99'}<#else>99</#if>" class="input small" datatype="n" sucmsg=" ">
  ...
  <dl>
    <dt>描述</dt>
    <dd>
      <textarea name="info" rows="2" cols="20" class="input" datatype="*0-255" sucmsg=" "><#if express_type??>${express_type.info!}</#if></textarea>
