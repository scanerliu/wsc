...
<body class="mainbody">
<form method="post" action="/management/article/list" id="form1">
...
<div id="floatHead" class="toolbar-wrap" style="height: 52px;">
  <div class="toolbar">
    <div class="box-wrap">
      ...
      <div class="l-list">
        <ul class="icon-list">
          <li><a class="add" href="/management/article/edit"><i></i><span>新增</span></a></li>
        ...
        <div class="menu-list">
          ...
          <div class="rule-single-select">
            <select name="ddlMoveId" onchange="javascript:setTimeout('__doPostBack(\'ddlMoveId\',\'\')', 0)" id="ddlMoveId" style="display: none;">
            	<option selected="selected" value="">批量移动...</option>
            	<#if category_list??>
              	<#list category_list as item>
                  <option value="${item.id?c}"><#if item.layerId!=1><#list 2..item.layerId as idx>　</#list>├ </#if>${item.title!''}</option>
                </#list>
            	</#if>
            </select>
          </div>
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="ltable">
    ...
    <#if article_page??>
      ...
      <tr>
        ...
        <td><a href="/management/article/edit?id=${item.id?c}">${item.title!''}</a></td>
        <td>${item.categoryTitle!''}</td>
        <td><#if item.createTime??>${item.createTime?string("yyyy-MM-dd HH:mm")}</#if></td>
        ...
        -->
        <td align="center">
          <a href="/management/article/edit?action=Copy&id=${item.id?c}">复制</a>
          <a href="/management/article/edit?id=${item.id?c}">修改</a>
        </td>
      </tr>
      </#list>
    </#if>
