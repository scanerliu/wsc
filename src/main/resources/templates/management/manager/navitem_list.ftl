...
<body class="mainbody">
<form method="post" action="/management/navitem/list" id="form1">
...
<div id="floatHead" class="toolbar-wrap" style="height: 52px;">
  <div class="toolbar">
    <div class="box-wrap">
      ...
      <div class="l-list">
        <ul class="icon-list">
          <li><a class="add" href="/management/navitem/edit"><i></i><span>新增</span></a></li>
        ...
        <div class="menu-list">
          <div class="rule-single-select single-select">
            <select name="categoryId" onchange="javascript:setTimeout('__doPostBack(\'ddlCategoryId\',\'\')', 0)" id="ddlCategoryId" style="display: none;">
            	...
            	<option value="0" <#if categoryId?? && categoryId==0> selected="selected"</#if> >触屏版导航菜单</option>
            	<option value="1" <#if categoryId?? && categoryId==1> selected="selected"</#if> >触屏版医生导航菜单</option>
            </select>
          </div>
        </div>
      </div>
...
<div class="table-container">
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="ltable">
    <tbody><tr class="odd_bg">
      ...
      <th align="center" width="12%">所属类别</th>
      <th align="left" width="65">排序</th>
      <th width="10%">操作</th>
    </tr>
    
    <#if navitem_page??>
      <#list navitem_page.content as item>
      <tr>
        <td align="center">
        ...
        <td><a href="/management/navitem/edit?id=${item.id?c}">${item.title!''}</a></td>
        <td align="center"><#if item.isEnable??><#if item.isEnable>开启<#else>关闭</#if></#if></td>
        <td align="center"><#if item.categoryId??><#if item.categoryId==0>触屏版导航菜单<#elseif item.categoryId==1>触屏版医生导航菜单</#if></#if></td>
        <td><input name="sortId" type="text" value="${item.sortId!''}" class="sort" onkeydown="return checkNumber(event);"></td>
        <td align="center">
          <a href="/management/navitem/edit?action=Copy&id=${item.id?c}">复制</a>
          <a href="/management/navitem/edit?id=${item.id?c}">修改</a>
        </td>
      </tr>
      </#list>
    </#if>
