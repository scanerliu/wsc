...
function codeCheck()
{
	...
	$.ajax({  
        ...
        success : function(data) {  
            ...
            if(data.status == 'y')
            {
                window.location.href= data.urlStr;
            }
            else
            {
                warning(data.info);
...
<section class="white_header">
    ...
    <div class="htitle header">绑定手机号</div>
...
<section class="pwdtextlinebox">
	<p style="margin: .6rem 0 .4rem 0; font-size: .36rem; color: #5e439b;">微信绑定</p>
    <form method="post">
    <span>
    ...
    <span>
        ...
        <input type="button" onclick="codeCheck()" value="绑 定" class="sub">
    </form>
</section>
<!-- pwdtextlinebox end -->
